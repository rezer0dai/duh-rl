...
class Normalizer(torch.nn.Module):
    ...
    def update(self, vv):
        if 1 == len(vv):
            return
        v = vv.clamp(-self.default_clip_range, self.default_clip_range)
    ...
    def normalize(self, v, clip_range=None):
        ...
        std = std.reshape(1, -1)
#        assert v.max() < 2.7 and -2.7 < v.min()
        return torch.clamp((v.clamp(-clip_range, clip_range) - mean) / std, -clip_range, clip_range)
